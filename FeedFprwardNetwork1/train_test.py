...
from hyperspectral_custom_datasetV1 import CustomDatasetV1
...
def train_loop(model, train_dataloader, loss_function, optimizer, device):
    ...
    train_loss = 0.0    #This variable keeps track of the cumulative training loss
    train_correct_predictions = 0 #This variable counts the number of correct predictions
    train_total_predictions = 0 #This variable counts the total number of predictions.
    ...
    for batches, (inputs, labels) in enumerate(train_dataloader):
        ...
        outputs = model(inputs) # This line computes the model's output by passing the input data through the model.
        loss = loss_function(outputs, labels) # This line calculates the loss between the model's output and the ground truth labels using the specified loss function.

        # Backpropagation and optimization
        optimizer.zero_grad()   # This line clears the gradients of all model parameters.
        loss.backward() # This line performs backpropagation by computing the gradients of the loss with respect to the model parameters.
        optimizer.step() # This line updates the model parameters based on the computed gradients and the chosen optimization algorithm.
...
if __name__ == "__main__":
    ...
    model_dataset = CustomDatasetV1(CSV_FILE)
    print(model_dataset)
    ...
    """
    In this modified code, we're using the AdamW optimizer (torch.optim.AdamW) instead of the Adam optimizer.
    AdamW is a variant of Adam optimizer that incorporates weight decay (L2 regularization) to help prevent
    overfitting. 
    """
    # optimizer = torch.optim.AdamW(my_modelV1.parameters(), lr=learning_rate)
